...
func TestServiceDescriber_DeploymentType(t *testing.T) {
	const (
		testApp               = "phonetool"
		testSvc               = "svc"
		testEnv               = "test"
		deploymentTypeDefault = "Default"
		abr                   = "Alarm-Based Rollback"
	)
	testCases := map[string]struct {
		setupMocks func(mocks ecsSvcDescriberMocks)

		wantedDeploymentType string
		wantedError          error
	}{
		"returns error if fails to get alarm names": {
			setupMocks: func(m ecsSvcDescriberMocks) {
				m.mockECSClient.EXPECT().Service(testApp, testEnv, testSvc).Return(nil, errors.New("some error"))
			},

			wantedError: errors.New("retrieve alarm names: get service svc: some error"),
		},
		"if alarm names present, type is 'Alarm-Based Rollback'": {
			setupMocks: func(m ecsSvcDescriberMocks) {
				gomock.InOrder(
					m.mockECSClient.EXPECT().Service(testApp, testEnv, testSvc).Return(&awsecs.Service{
						DeploymentConfiguration: &ecsapi.DeploymentConfiguration{
			...
			wantedDeploymentType: abr,
		},
		"if no alarm names present, type is 'Default'": {
			setupMocks: func(m ecsSvcDescriberMocks) {
				gomock.InOrder(
					m.mockECSClient.EXPECT().Service(testApp, testEnv, testSvc).Return(&awsecs.Service{
						DeploymentConfiguration: &ecsapi.DeploymentConfiguration{},
			...
			wantedDeploymentType: deploymentTypeDefault,
		},
	}
	...
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			if tc.wantedError != nil {
				...
				require.NoError(t, err)
				require.Equal(t, tc.wantedDeploymentType, actual)
			}
		})
	}
}
