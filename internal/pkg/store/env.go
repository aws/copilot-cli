...
func (s *Store) CreateEnvironment(environment *archer.Environment) error {
	if _, err := s.GetApplication(environment.Project); err != nil {
		return err
	}
	...
	if err != nil {
		...
		return fmt.Errorf("create environment %s in application %s: %w", environment.Name, environment.Project, err)
	}
	return nil
}
...
func (s *Store) GetEnvironment(appName string, environmentName string) (*archer.Environment, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("read details for environment %s in application %s: %w", environmentName, appName, err)
	}
	return &env, nil
}

// ListEnvironments returns all environments belonging to a particular application.
func (s *Store) ListEnvironments(appName string) ([]*archer.Environment, error) {
	...
	for _, serializedEnv := range serializedEnvs {
		...
		if err := json.Unmarshal([]byte(*serializedEnv), &env); err != nil {
			return nil, fmt.Errorf("read environment details for application %s: %w", appName, err)
		}
