...
func TestValidateTopicARN(t *testing.T) {
	testCases := map[string]struct {
		inAccount string
		inRegon   string
		inApp     string
		...
		"empty account info": {
			...
			inEnv:     "environment",
			inSvc:     "svc",
			wantErr:   errInvalidARNAccountInfo,
		},
		"empty svc info": {
			inAccount: "123456789012",
			inRegon:   "us-west-2",
			...
			inEnv:     "environment",
			inSvc:     "",
			wantErr:   errInvalidARNSvcInfo,
		},
	}

	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			err := validateARNParemeters(tc.inAccount, tc.inRegon, tc.inApp, tc.inEnv, tc.inSvc)
