...
func TestRollbackAlarmArgsOrNamesTransformer_Transformer(t *testing.T) {
	mockAlarmNames := []*string{aws.String("alarm1"), aws.String("alarm2")}
	testCases := map[string]struct {
		original func(r *RollbackAlarmArgsOrNames)
		override func(r *RollbackAlarmArgsOrNames)
		wanted   func(r *RollbackAlarmArgsOrNames)
	}{
		"string slice set to empty if args is not nil": {
			original: func(r *RollbackAlarmArgsOrNames) {
				r.AlarmNames = mockAlarmNames
			},
			override: func(r *RollbackAlarmArgsOrNames) {
				r.AlarmArgs = AlarmArgs{
	...
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			var dst, override, wanted RollbackAlarmArgsOrNames
			...
			// Use custom transformer.
			err = mergo.Merge(&dst, override, mergo.WithOverride, mergo.WithTransformers(rollbackAlarmArgsOrNamesTransformer{}))
			require.NoError(t, err)
