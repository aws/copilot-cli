...
func Test_GenerateConditionGroups(t *testing.T) {
	tests := map[string]struct {
		inWkldType string
		isHTTPS    bool
		inALBRule  ALBListenerRule
		wanted     []conditionGroup
	}{
		"Load Balanced service with HTTP listener rule and root path / ": {
			inWkldType: "Load Balanced Web Service",
			inALBRule: ALBListenerRule{
				Path: "/",
				AllowedSourceIps: []string{
					"192.0.2.0/24", "198.51.100.10/32", "200.51.100.10/28", "255.51.100.10/28", "200.31.100.10/24",
				},
				Aliases: []string{"v1.copilot.com", "v2.copilot.com", "v3.copilot.com"},
			},
			wanted: []conditionGroup{
				{
					AllowedSourceIps: []string{"192.0.2.0/24", "198.51.100.10/32", "200.51.100.10/28", "255.51.100.10/28"},
				},
				{
					AllowedSourceIps: []string{"200.31.100.10/24"},
				},
			},
		},
		"Load Balanced service with HTTP listener rule and non root path": {
			inWkldType: "Load Balanced Web Service",
			inALBRule: ALBListenerRule{
				Path: "admin",
				AllowedSourceIps: []string{
			...
			wanted: []conditionGroup{
				{
					AllowedSourceIps: []string{"192.0.2.0/24", "198.51.100.10/32", "200.51.100.10/28"},
				},
				{
					AllowedSourceIps: []string{"255.51.100.10/28", "200.31.100.10/24"},
				},
			},
		},
		"Backend Service with HTTP Listener rule and root path / ": {
			inWkldType: "Backend Service",
			inALBRule: ALBListenerRule{
				Path:             "/",
				AllowedSourceIps: []string{"192.0.2.0/24", "198.51.100.10/32", "200.51.100.10/28", "255.51.100.10/28", "200.31.100.10/24"},
				Aliases:          []string{"v1.copilot.com", "v2.copilot.com", "v3.copilot.com"},
			},
			wanted: []conditionGroup{
				{
					AllowedSourceIps: []string{"192.0.2.0/24", "198.51.100.10/32"},
				},
				{
					AllowedSourceIps: []string{"200.51.100.10/28", "255.51.100.10/28"},
				},
				{
					AllowedSourceIps: []string{"200.31.100.10/24"},
				},
		...
		"Backend Service with HTTP Listener rule and non root path": {
			inWkldType: "Backend Service",
			inALBRule: ALBListenerRule{
				Path: "admin",
				AllowedSourceIps: []string{
					"192.0.2.0/24", "198.51.100.10/32", "200.51.100.10/28",
				},
				Aliases: []string{"v1.copilot.com", "v2.copilot.com", "v3.copilot.com"},
			},
			wanted: []conditionGroup{
				{
					AllowedSourceIps: []string{"192.0.2.0/24"},
				},
				{
					AllowedSourceIps: []string{"198.51.100.10/32"},
				},
				{
					AllowedSourceIps: []string{"200.51.100.10/28"},
				},
			},
		},
		"load balanced or backend service with HTTPS Listener with only allowed Source IPs and root path": {
			inALBRule: ALBListenerRule{
				Path: "/",
				AllowedSourceIps: []string{
					"192.0.2.0/24", "198.51.100.10/32", "200.51.100.10/28", "255.51.100.10/28", "200.31.100.10/24",
		...
		"load balanced or backend service with HTTPS Listener with only allowed Source IPs and non root path": {
			inALBRule: ALBListenerRule{
				Path: "admin",
				AllowedSourceIps: []string{
					"192.0.2.0/24", "198.51.100.10/32", "200.51.100.10/28", "255.51.100.10/28", "200.31.100.10/24",
		...
		"load balanced or backend service with HTTPS Listener with only alias and root path": {
			isHTTPS: true,
			inALBRule: ALBListenerRule{
				Path:    "/",
				Aliases: []string{"v1.copilot.com", "v2.copilot.com", "v3.copilot.com", "v4.copilot.com", "v5.copilot.com", "v6.copilot.com"},
			},
			wanted: []conditionGroup{
				{
					Aliases: []string{"v1.copilot.com", "v2.copilot.com", "v3.copilot.com", "v4.copilot.com"},
				},
				{
					Aliases: []string{"v5.copilot.com", "v6.copilot.com"},
				},
			},
		},
		"load balanced or backend service with HTTPS Listener with only alias and non root path": {
			isHTTPS: true,
			inALBRule: ALBListenerRule{
				Path:    "admin",
				Aliases: []string{"v1.copilot.com", "v2.copilot.com", "v3.copilot.com", "v4.copilot.com", "v5.copilot.com", "v6.copilot.com"},
			...
			wanted: []conditionGroup{
				...
				{
					Aliases: []string{"v4.copilot.com", "v5.copilot.com", "v6.copilot.com"},
				},
			},
		},
		"load balanced or backend service with HTTPS Listener with both alias and allowed source IPs and root path": {
			isHTTPS: true,
			inALBRule: ALBListenerRule{
				Path:             "/",
				AllowedSourceIps: []string{"192.0.2.0/24", "198.51.100.10/32"},
				Aliases:          []string{"v1.copilot.com", "v2.copilot.com", "v3.copilot.com", "v4.copilot.com", "v5.copilot.com", "v6.copilot.com", "v7.copilot.com"},
			},
			wanted: []conditionGroup{
				{
					AllowedSourceIps: []string{"192.0.2.0/24"},
		...
		"load balanced or backend service with HTTPS Listener with both alias and allowed source IPs and non root path": {
			isHTTPS: true,
			inALBRule: ALBListenerRule{
				Path:             "admin",
				AllowedSourceIps: []string{"192.0.2.0/24", "198.51.100.10/32"},
			...
			wanted: []conditionGroup{
				{
					AllowedSourceIps: []string{"192.0.2.0/24"},
					Aliases:          []string{"v1.copilot.com", "v2.copilot.com"},
				},
				{
					AllowedSourceIps: []string{"192.0.2.0/24"},
					Aliases:          []string{"v3.copilot.com", "v4.copilot.com"},
				},
				{
					AllowedSourceIps: []string{"192.0.2.0/24"},
					Aliases:          []string{"v5.copilot.com", "v6.copilot.com"},
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			require.Equal(t, tc.wanted, tc.inALBRule.GenerateConditionGroups(tc.inWkldType, tc.isHTTPS))
		})
	}
}
