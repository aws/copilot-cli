...
func TestTemplate_ParseDeploymentConfig(t *testing.T) {
	type cfn struct {
		Resources struct {
			Service struct {
				Properties struct {
					DeploymentConfiguration map[interface{}]interface{} `yaml:"DeploymentConfiguration"`
				} `yaml:"Properties"`
			} `yaml:"Service"`
		} `yaml:"Resources"`
	}
	...
	testCases := map[string]struct {
		input template.DeploymentConfigurationOpts

		wantedDeploymentConfig string
	}{
		"should render existing alarm names": {
			input: template.DeploymentConfigurationOpts{
				MinHealthyPercent: 100,
				MaxPercent:        200,
				RollbackAlarms:    "alarm1, alarm2",
			},
...
`,
		...
		"should use Copilot-created names if creating alarms": {
			input: template.DeploymentConfigurationOpts{
				MinHealthyPercent: 100,
				MaxPercent: 200,
				CPUUtilization: 32,
...
`,
		...
		"should only list one alarm name if only creating one": {
			input: template.DeploymentConfigurationOpts{
				MinHealthyPercent: 100,
				MaxPercent: 200,
				MemoryUtilization: 65,
...
`,
	...
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			tpl := template.New()
			wanted := make(map[interface{}]interface{})
			err := yaml.Unmarshal([]byte(tc.wantedDeploymentConfig), &wanted)
			require.NoError(t, err, "unmarshal wanted config")

			// WHEN
			content, err := tpl.ParseLoadBalancedWebService(template.WorkloadOpts{DeploymentConfiguration: tc.input})
			...
			err = yaml.Unmarshal(content.Bytes(), &actual)
			require.NoError(t, err, "unmarshal actual config")
			require.Equal(t, wanted, actual.Resources.Service.Properties.DeploymentConfiguration)
		})
	}
}
