...
func TestAddons(t *testing.T) {
	testCases := map[string]struct {
		...
		"aurora": {
			addonMarshaler: addon.NewWorkloadServerlessV2Template(addon.RDSProps{
				ClusterName:   "aurora",
				Engine:        "MySQL",
				InitialDBName: "main",
				Envs:          []string{"test"},
		...
		"ddb": {
			addonMarshaler: addon.NewWorkloadDDBTemplate(&addon.DynamoDBProps{
		...
		"s3": {
			addonMarshaler: addon.NewWorkloadS3Template(&addon.S3Props{
				StorageProps: &addon.StorageProps{
					Name: "bucket",
				},
			}),
