...
type workspaceAddonReader interface {
...
func ParseFromWorkload(workloadName string, ws workspaceAddonReader) (*WorkloadStack, error) {
	parser := parser{
		ws: ws,
		addonsDirPath: func() string {
...
// files found there. If no addons are found, ParseFromWorkload returns a nil
// Stack and ErrAddonsNotFound.
func ParseFromEnv(ws workspaceAddonReader) (*EnvironmentStack, error) {
	parser := parser{
		ws:             ws,
