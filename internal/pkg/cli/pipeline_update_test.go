...
func TestUpdatePipelineOpts_ConnectionName(t *testing.T) {
	testCases := map[string]struct {
		inRepoURL              string
		expectedConnectionName string
		expectedError          error
	}{
		"errors if repoURL empty": {
			inRepoURL:     "",
			expectedError: errors.New("unable to locate the repository URL"),
		},
		"errors if can't parse URL": {
			inRepoURL:     "unparsableURL",
			expectedError: errors.New("unable to parse the repository URL"),
		},
		"successfully parses URL and generates connection name": {
			inRepoURL:              "https://huanjani@bitbucket.org/huanjani/aws-copilot-sample-service",
			expectedConnectionName: "copilot-huanj-aws-copilot-sample",
		},
	}
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			if tc.expectedError != nil {
				...
				require.NoError(t, err)
				require.Contains(t, tc.expectedConnectionName, actual)
			}
		})
	}
}
