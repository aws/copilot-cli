...
func NewBackendDeployer(in *WorkloadDeployerInput) (*backendSvcDeployer, error) {
	...
	if !ok {
		return nil, fmt.Errorf("manifest is not of type %s", manifesttype.BackendServiceType)
	}
...
func backendCustomResources(fs template.Reader) ([]*customresource.CustomResource, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("read custom resources for a %q: %w", manifesttype.BackendServiceType, err)
	}
	return crs, nil
}
