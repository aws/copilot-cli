...
func TestWorkloadDeployer_DeployWorkload(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"fail to enable https alias because of incompatible app version": {
			...
			wantErr: fmt.Errorf("validate ALB runtime configuration for \"http\": alias not supported: app version must be >= %s", version.AliasLeastAppTemplateVersion),
		},
		"fail to enable nlb alias because of incompatible app version": {
			...
			wantErr: fmt.Errorf("alias not supported: app version must be >= %s", version.AliasLeastAppTemplateVersion),
		},
