...
func TestWorkloadDeployer_DeployWorkload(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"fail to validates condition values per listener rule": {
			...
			wantErr: fmt.Errorf(`validate ALB runtime configuration for "http": validate condition values per listener rule:Listener Rule can not have more than five Condition Values example.com, v1.example.com, v2.example.com, v3.example.com and v4.example.com `),
		},
