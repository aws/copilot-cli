...
var (
	...
	errRDWSNotConnectedToVPC       = fmt.Errorf("%s requires a VPC connection", manifesttype.RequestDrivenWebServiceType)
...
func validateSvcType(val interface{}) error {
	...
	return validateWorkloadType(svcType, manifesttype.ServiceTypes(), service)
}
...
func validateJobType(val interface{}) error {
	...
	return validateWorkloadType(jobType, manifesttype.JobTypes(), job)
}
...
func validateAuroraStorageType(ws manifestReader, workloadName string) error {
	...
	if mftType != manifesttype.RequestDrivenWebServiceType {
		return nil
	}
	data := struct {
