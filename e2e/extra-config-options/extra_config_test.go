...
var _ = Describe("extra config flow", func() {
	...
	Context("when adding a svc", func() {
		...
		BeforeAll(func() {
			...
			srcFile, err := os.Open(fmt.Sprintf("./%s/manifest.yml", svcName))
			Expect(err).NotTo(HaveOccurred(), "open source file")
			defer srcFile.Close()

			_, err = io.Copy(destFile, srcFile)
			Expect(err).NotTo(HaveOccurred(), "copy manifest file")
	...
	Context("when deploying svc", func() {
		...
		It("should get the overwritten env var", func() {
			...
			Expect(err).NotTo(HaveOccurred())
			Expect(bodyBytes).To(Equal("open sesame"))
		})
