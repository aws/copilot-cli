...
var _ = Describe("Multiple Service App", func() {
	...
	Context("when adding a svc", func() {
		...
		It("svc ls should list the svc", func() {
			...
			for _, svc := range []string{"front-end", "back-end"} {
	...
	Context("when deploying services", func() {
		...
		It("svc show should include a valid URL and description for test and prod envs", func() {
			for _, svcName := range []string{"front-end", "back-end"} {
				...
				// Since the back-end was added second, it should have the svc name appended
				// to the route
				if svcName == "back-end" {
					Expect(route.URL).To(HaveSuffix(svcName))
				}
